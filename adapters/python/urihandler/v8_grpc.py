...
Client `call()` mirrors `v8_service.call`: it validates the payload against the
...
def serve(registry: dict, host: str = "0.0.0.0", port: int = DEFAULT_PORT, policy: dict | None = None,
    ...
    def do_run(request, _context):
        return v8.run(request["uri"], registry, payload=request.get("payload"),
                      mode=request.get("mode") or mode, policy=policy)

    def do_run_stream(request, _context):
        yield {"event": "start", "uri": request["uri"]}
        yield {"event": "result", **v8.run(request["uri"], registry, payload=request.get("payload"),
                                           mode=request.get("mode") or mode, policy=policy)}
...
def _validate(uri: str, payload: dict, registry: dict | None) -> dict | None:
    ...
    try:
        v8.validate_input(route_entry, descriptor, translation, payload or {})
...
def main(argv: list[str] | None = None) -> int:
    ...
    s.add_argument("source", help="project dir, v8 bindings file, or registry document")
