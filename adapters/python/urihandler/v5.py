...
from urihandler import v4

BINDINGS_VERSION = "urihandler.bindings.v5"
...
def normalize_binding(binding: dict, default_source: dict | None = None) -> dict:
    ...
    normalized = {
        ...
        "adapter": binding.get("adapter") or v4.default_adapter(infer_kind(binding)),
        "config": config,
        "source": source,
    }
...
def load_bindings_from_manifest(data, source: dict | None = None) -> list[dict]:
    ...
    return [route_source_to_binding(route) for route in v4.discover_manifest(data, default_source)]
...
def compile_registry_document(binding_document_or_bindings, generated_at: str | None = None, on_conflict: str = "keep") -> dict:
    ...
    return v4.build_registry_document(
...
def _read_toml(path: Path) -> dict:
    ...
    # sections used by v5 discovery and ignores everything else.
    data: dict = {}
    current: list[str] = []
...
def scan_docker_compose(path: Path, root: Path) -> list[dict]:
    ...
    for service, labels in labels_by_service.items():
        if not labels:
            continue
        for route in v4.discover_docker_labels(labels, {"type": "docker-compose-label", "file": relpath(path, root), "service": service}):
...
def scan_openapi(path: Path, root: Path, base_url: str = "") -> list[dict]:
    ...
    return [
        ...
        for route in v4.discover_openapi(data, base_url=base_url, source={"file": relpath(path, root)})
    ]
...
def scan_github(repo: str, include_shell: bool = True, openapi_base_url: str = "") -> list[dict]:
    with tempfile.TemporaryDirectory(prefix="urihandler-v5-github-") as tmp:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] in {"discover", "build-registry"}:
        return v4.main(argv)

    parser = argparse.ArgumentParser(prog="urihandler")
    subparsers = parser.add_subparsers(dest="command", required=True)

    scan = subparsers.add_parser("scan", help="Scan a project and generate v5 bindings")
    scan.add_argument("path")
    scan.add_argument("--out", default=".urihandler/bindings.v5.json")
    ...
    scan_github_parser.add_argument("--out", default=".urihandler/bindings.v5.json")
