...
def _ensure_cli_bridge(name: str):
    """Return the CLI bridge fn for ``name``; if absent, import the node layer (app-side) to fire
    its module-level _register_cli_bridge(), then re-check. The kernel never imports node at module
    scope (keeps the extraction boundary clean) — this lazy import is the CLI entry triggering the
    seam exactly when a host/node subcommand needs it, so `urirun node serve` works from a cold CLI."""
