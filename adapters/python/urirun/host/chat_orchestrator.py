...
def _chat_ask_general_check_offline(
    ...
    from ._orchestrator_offline import (  # noqa: PLC0415
        _offline_nodes_from_diagnosis, _escalate_offline_to_human, _emit_offline_escalation_message,
    )
    if not selected_nodes:
        return None
...
_MOVED_TO_ORCHESTRATOR_OFFLINE: frozenset[str] = frozenset({
    "_find_human_node", "_resolve_node_remediation", "_diagnosis_title",
    "_diagnosis_instruction", "_diagnosis_error_type", "_extract_node_diagnosis_params",
    "_build_dryrun_offline_envelope", "_build_no_route_offline_envelope",
    "_build_executed_offline_envelope", "_escalate_offline_to_human",
    "_emit_offline_escalation_message", "_offline_nodes_from_diagnosis",
})


def __getattr__(name: str):
    if name in _MOVED_TO_ORCHESTRATOR_OFFLINE:
        from urirun.host import _orchestrator_offline  # noqa: PLC0415
        return getattr(_orchestrator_offline, name)
