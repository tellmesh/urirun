...
so ``from urirun.node.mesh import EventHub`` / ``mesh.serve_node(…)`` etc.
...
from urirun.node.routing import registry_fingerprint, routes_from_registry
from urirun.node._version import current_version, version_line, version_status
