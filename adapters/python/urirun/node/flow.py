...
def _flow_timeline_entry(step: dict, env: dict, routes: list[dict], *, attempt: int = 0,
    ...
    # Mirror _thin_step_entry: read-only steps (/query/) are trivially reversible.
...
def _preflight_cdp(flow: dict, registry: dict, entries: list) -> None:
    """Proactively provision CDP sessions for any CDP-targeted step in the flow."""
...
def _preflight_preconditions(flow: dict, entries: list) -> None:
    """Surface human-gated capture preconditions as advisory preflight entries."""
    try:
        from urirun.node import preconditions as _pc  # noqa: PLC0415
        from urirun.node.twin_store import durable_memory as _durable_memory  # noqa: PLC0415
        from urirun.node.reversible import environment_fingerprint  # noqa: PLC0415
        kvm_targets = sorted({
            route_target(str(s.get("uri") or ""))
            for s in flow.get("steps") or []
            if str(s.get("uri") or "").startswith("kvm://")
            and "/screen/query/capture" in str(s.get("uri") or "")
        })
        for target in kvm_targets:
            _preflight_preconditions_for_target(target, flow, entries, _pc, _durable_memory,
                                                environment_fingerprint)
    except Exception:  # noqa: BLE001 — precondition check must not crash preflight
        pass


def _preflight_preconditions_for_target(target, flow, entries, _pc, _durable_memory,
                                         environment_fingerprint) -> None:
    """Check and surface capture preconditions for a single kvm target."""
    if not target:
        return
    prof = _fetch_env_profile({"uri": f"kvm://{target}/_"}, {})
    env_fp = environment_fingerprint(prof) if prof else ""
    mem = _durable_memory()
    capture_preconditions = [p for p, pc_obj in _pc._REGISTRY.items()
                             if "capture" in (pc_obj.tags or ())]
    if not capture_preconditions:
        return
    report = _pc.readiness_report(capture_preconditions, env_fingerprint=env_fp,
                                  memory=mem, node=target)
    if not report.get("ready"):
        if report.get("auto"):
            report = _pc.apply_auto(report, node=target, memory=mem, env_fingerprint=env_fp)
        for hg in report.get("humanGated") or []:
            entries.append({
                "id": f"preflight:precondition:{hg['precondition']}",
                "uri": f"kvm://{target}/screen/query/capture",
                "target": target, "ok": False, "type": "precondition",
                "action": "acquire-human-gated",
                "precondition": hg["precondition"],
                "hint": hg.get("hint") or hg.get("description") or "",
                "humanGated": True,
            })
...
def _preflight(flow: dict, registry: dict) -> list[dict]:
    ...
    Also runs a readiness check for kvm capture steps — surfaces unsatisfied preconditions
    (e.g. missing portal grant) as preflight entries with ``humanGated`` actions, so the
    caller can surface them to the user before the flow starts rather than discovering them
    mid-run as a degraded result."""
    entries: list[dict] = []
    _preflight_cdp(flow, registry, entries)
    _preflight_preconditions(flow, entries)
    return entries
...
def _uri_preflight(payload: dict) -> dict:
    ...
    # Separate human-gated precondition entries: they are advisory (not hard failures)
    # but must be surfaced to the user before the flow starts.
    precondition_entries = [e for e in entries if e.get("type") == "precondition"]
    surface_entries = [e for e in entries if e.get("type") != "precondition"]
    all_ok = all(e.get("ok", True) for e in surface_entries)
    human_gated = [{"precondition": e["precondition"], "hint": e.get("hint", "")}
                   for e in precondition_entries if e.get("humanGated")]
    return {
        **urirun.ok(ok=all_ok),
        "timeline": entries,
        "count": len(entries),
        **({"readinessReport": {"humanGated": human_gated, "ready": not human_gated}}
           if precondition_entries else {}),
    }
