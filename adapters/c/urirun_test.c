...
int main(void) {
  ...
  assert(urirun_parse("device://device-01/led/set/on?trace=1#ui", &d) == 0);
  ...
  assert(urirun_parse("not-a-uri", &d) != 0);
  assert(urirun_parse(NULL, &d) != 0);
  assert(urirun_parse("device://target/a/b/c/d/e/f/g/h/i", &d) != 0);
  return 0;
}
