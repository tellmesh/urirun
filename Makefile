...
.PHONY: test-v7
test-v7: ## Run urirun v7 parameter-binding, docker, and shell checks.
	...
	$(PYTHON) -m json.tool v7/examples/json/bindings.v7.example.json >/tmp/urirun-v7-bindings.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v7 compile v7/examples/json/bindings.v7.example.json --out /tmp/urirun-v7.registry.json --generated-at 2026-06-19T00:00:00.000Z
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v7 run 'media://local/video/transcode' --registry /tmp/urirun-v7.registry.json --payload '{"input":"a.mp4","output":"b.mp4"}' >/tmp/urirun-v7-ffmpeg.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v7 list /tmp/urirun-v7.registry.json --allow 'media://**'
	$(NODE) v7/examples/html_uri_app/test.mjs

.PHONY: test-v8
test-v8: ## Run urirun v8 schema/decorator, artifact, and MCP/A2A checks.
	PYTHONPATH=adapters/python $(PYTHON) -m unittest discover -s v8/examples/python -p 'test_*.py'
	$(NODE) v8/examples/generators/nodejs/generate-bindings.mjs >/tmp/urirun-v8-gen.json
	$(NODE) v8/examples/html_uri_app/test.mjs
	$(PYTHON) -m json.tool v8/examples/json/bindings.v8.example.json >/tmp/urirun-v8-bindings.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v8 compile v8/examples/json/bindings.v8.example.json --out /tmp/urirun-v8.registry.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v8_mcp tools /tmp/urirun-v8.registry.json >/tmp/urirun-v8-mcp.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v8_mcp card /tmp/urirun-v8.registry.json >/tmp/urirun-v8-a2a.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v8_adopt add-python-package pip --out /tmp/urirun-v8-adopt.bindings.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v8 compile /tmp/urirun-v8-adopt.bindings.json --out /tmp/urirun-v8-adopt.registry.json
	PYTHONPATH=adapters/python $(PYTHON) -m urirun.v8 run 'cli://pip/pip/run' --registry /tmp/urirun-v8-adopt.registry.json --payload '{"args":["--version"]}' >/tmp/urirun-v8-adopt-run.json
	command -v php >/dev/null 2>&1 && php v8/examples/generators/php/example.php >/tmp/urirun-v8-php.json || echo "php not installed; skipping PHP generator"
