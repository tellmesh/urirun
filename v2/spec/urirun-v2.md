...
`urirun v2` turns command endpoints into schema-first packages.
...
| `docker-run` | inherited from v1, renders command inside `docker run` |
| `docker-exec` | inherited from v1, renders command inside `docker exec` |
...
v2 can scan a project directory and adopt common declarations:
...
- `bindings.v2.json` - the schema-first package contract, suitable for review
...
v2 can append bindings without hand-editing JSON:
...
The command updates or creates a standard v2 bindings document, so the next
step is still the normal compile/run flow.

## Adopting installed package commands

Decorators cover code you own. `urirun.v2_adopt` covers code you *install* -
...
python -m urirun.v2_adopt init . --out urirun.bindings.v2.json --registry-out .urirun/registry.merged.json
```
...
`urirun.bindings.v2` JSON document.

## Interop: MCP and A2A (agent discovery)

Because a v2 binding already carries a JSON Schema (`inputSchema`), the registry
...
urirun registry        ->  MCP tools/list   (LLM tool calling)
                       ...
                       ->  tools/call        ->  v2 policy gate -> run
```

`urirun.v2_mcp` provides the projection and a minimal MCP server:

```bash
python -m urirun.v2_mcp tools bindings.v2.example.json     # MCP tool manifest
python -m urirun.v2_mcp card  bindings.v2.example.json     # A2A agent card
python -m urirun.v2_mcp serve registry.json               # MCP stdio server (dry-run)
python -m urirun.v2_mcp serve registry.json --execute --policy policy.json
```
...
```bash
urirun scan . --out generated/bindings.v2.json --registry-out generated/registry.json
urirun validate generated/bindings.v2.json
urirun list generated/registry.json
```
...
1. scan artifacts into `generated/bindings.v2.json`;
