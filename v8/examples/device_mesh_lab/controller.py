...
def build_registry(routes: list[dict]) -> dict:
    ...
    if v8 is None:
        return {"version": "urirun.bindings.v8", "bindings": bindings, "index": {uri: {"uri": uri} for uri in bindings}}
    return v8.compile_registry({"version": "urirun.bindings.v8", "bindings": bindings})
...
def execute_flow(flow: dict, mesh: dict, registry: dict) -> dict:
    ...
    for step in flow["steps"]:
        ...
        if v8_service is None:
            ...
            env = v8_service.call(step["uri"], step.get("payload") or {}, registry, mode="execute", timeout=20)
        results[step["id"]] = env
        timeline.append({
            "id": step["id"],
            "uri": step["uri"],
